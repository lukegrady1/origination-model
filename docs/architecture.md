...
- Feature caches are keyed by source hashes, first-observed metadata, the requested seasons,
  policy, feature config and feature/schema version,
...
- Availability fields (`insufficient_warmup`, `unobserved_inputs`) and the current feature
  ...
  permission to continue. Flags must be non-null booleans; strings and numeric substitutes are
  rejected instead of coerced.
- A team-game row's observation time is known only when both its schedule and play-by-play
  observation times are known; unknown means ineligible under `recorded_asof`. Every contributing
  eligible play must have an observation timestamp, and both offenses must have complete
  observation metadata, because each perspective uses the opponent's offense for defensive
  metrics. A group maximum must never hide an unknown observation on another play or team.
